// Boundary condition UI: tap-to-select + bottom-sheet editor. Incompressible BCs.
(function () {
  ...
  const TYPES = {
    ...
    top:      ["slip", "symmetry", "no_slip"],
    bottom:   ["slip", "symmetry", "no_slip"],
    obstacle: ["no_slip"],
  };
  const FIELDS = {
    inlet_velocity:  [["speed", "Speed"], ["angle_deg", "Angle [deg]"]],
    outlet_pressure: [["p", "Pressure"]],
    slip:            [],
    symmetry:        [],
    no_slip:         [],
  };
  ...
  function bcSummary(bc) {
    if (!bc) return "";
    if (bc.type === "inlet_velocity")
      return `U=${bc.speed}, ${bc.angle_deg || 0}°`;
    if (bc.type === "outlet_pressure") return `p=${bc.p}`;
  ...
  function renderFields(type, bc) {
    ...
    wrap.innerHTML = "";
    const defaults = { speed: 1.0, angle_deg: 0.0, p: 0.0 };
